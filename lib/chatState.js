...
// Effect dependency (trigger.dependencies: [{effectId, minLevel}]): AND-gate — every entry must
// be satisfied for the whole prerequisite to count as met. Fail-open per entry on a broken
// reference (deleted/renamed-away effect) — an unmet-but-unresolvable dependency shouldn't
// permanently block an effect, it should just drop out of consideration (a separate caution
// indicator in the UI flags the broken reference instead, see describeDependencyState below).
...
export function isPrerequisiteMet(effect, allEffects) {
    return (effect.trigger.dependencies ?? []).every(dep => {
        const prerequisite = allEffects.find(e => e.id === dep.effectId);
        if (!prerequisite) return true;
        return meetsDirectionalThreshold(getEffectLevel(prerequisite), dep.minLevel, prerequisite.trigger.hitDirection);
    });
}
...
// null when there's nothing to show (no dependencies configured, or every one is satisfied).
// `broken` is true if ANY entry is broken (worst case wins, same as a single broken dependency
// used to make the whole thing "broken"); `reason` joins one line per broken/blocked entry.
export function describeDependencyState(effect, allEffects) {
    ...
    for (const dep of dependencies) {
        ...
        if (!prerequisite) {
            broken = true;
            lines.push(`Depends on an effect that no longer exists (id "${dep.effectId}") — treated as no dependency for now.`);
            continue;
        }
        const prerequisiteLevel = getEffectLevel(prerequisite);
        if (meetsDirectionalThreshold(prerequisiteLevel, dep.minLevel, prerequisite.trigger.hitDirection)) continue;
        lines.push(
