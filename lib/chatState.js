...
const GLOBAL_AWARENESS_LEVEL_KEY = 'st_mangler_global_awareness_level';

// Global "character awareness" (settings.globalAwareness) — a single chat-scoped value, not
// tracker/effect-scoped (same flat-key shape as TRANSFORM_PAUSED_KEY above, unlike
// trackerLevelKey/trackerTurnsKey/etc. elsewhere in this file which all interpolate a tracker id).
// See pipeline.js's applyEffects Phase A and lib/llmClient.js's applyLlmRating for what updates it.
export function getGlobalAwarenessLevel() {
    return getChatMetadata()[GLOBAL_AWARENESS_LEVEL_KEY] ?? 0;
}

export function setGlobalAwarenessLevel(level) {
    getChatMetadata()[GLOBAL_AWARENESS_LEVEL_KEY] = level;
    context.saveMetadataDebounced();
    return level;
}
