...
import { getEffectLevel, setEffectLevel, getEffectLocked, setEffectLocked } from './chatState.js';
import {
...
export function applyLlmRating(effect, rating0to10) {
    ...
    const { level: newLevel, locked: newLocked } = resolveLlmRatingUpdate(
        getEffectLevel(effect), wasLocked, rating0to10, effect.trigger,
    );
...
export async function runBatchedLlmDetectors(effects) {
