...
import {
    getTrackerLevel, setTrackerLevel, getTrackerLocked, setTrackerLocked, isPrerequisiteMet,
...
import {
    ...
    buildChainPreservationNote, resolveGlobalAwarenessHit, resolveAmountStep,
} from './pure.js';
import { logEvent } from './eventLog.js';

// Soft warning threshold for runBatchedLlmDetectors below — packing many conditions into one
// classification prompt has the same "model misses/confuses some instructions" risk as one
// complex prompt; not a hard cap (there's no cost/latency reason to refuse the call, unlike
// MANY_ACTIVE_REWRITES_WARNING_THRESHOLD in pipeline.js), just a flag that accuracy may degrade.
const MANY_BATCHED_CONDITIONS_WARNING_THRESHOLD = 6;
...
// Returns { level, hit } — `hit` lets runBatchedLlmDetectors below aggregate across every tracker
// in one batch and bump global "character awareness" at most once per batch, same one-per-message
// cap Phase A applies to keyword hits (pipeline.js), rather than compounding per hitting tracker.
export function applyLlmRating(tracker, rating0to10, allTrackers = []) {
    ...
    if (tracker.llmIntegrationMode === 'cumulative-lock' && wasLocked) {
        ...
        return { level: getTrackerLevel(tracker), hit: false };
    }

    const levelBefore = getTrackerLevel(tracker);
    const { level: newLevel, locked: newLocked, hit } = resolveLlmRatingUpdate(
        levelBefore, wasLocked, rating0to10, tracker, prerequisiteMet,
    );
    const level = setTrackerLevel(tracker, newLevel);
    debugLog(`applyLlmRating "${tracker.label}": -> level=${level.toFixed(2)}`);
    if (hit) {
        logEvent(tracker.id, 'level-change', { from: levelBefore, to: level, reason: 'LLM rating' });
    }
    if (newLocked && !wasLocked) {
        ...
        logEvent(tracker.id, 'lock', { level, lockThreshold: tracker.lockThreshold });
    }
    return { level, hit };
}
...
export async function runBatchedLlmDetectors(trackers, allTrackers = trackers, globalAwareness = null) {
    if (trackers.length === 0) return;
    debugLog(`runBatchedLlmDetectors: firing for ${trackers.length} tracker(s): ${trackers.map(t => t.label).join(', ')}`);
    if (trackers.length >= MANY_BATCHED_CONDITIONS_WARNING_THRESHOLD) {
        warn(`Batching ${trackers.length} conditions into one LLM detector call — the model may start missing or confusing some at this count. Consider trimming which trackers are due at once, or splitting into separate detector connection profiles.`);
    }
    ...
    try {
        ...
        // anyHit aggregates across every tracker in this one batch — global awareness bumps at
        // most once per batch (same one-per-message cap Phase A applies to keyword hits), not
        // once per hitting tracker.
        let anyHit = false;
        for (const tracker of trackers) {
            ...
            if (rating !== null) {
                const result = applyLlmRating(tracker, rating, allTrackers);
        ...
        if (anyHit && globalAwareness?.enabled) {
            setGlobalAwarenessLevel(resolveGlobalAwarenessHit(getGlobalAwarenessLevel(), globalAwareness.incrementPerHit));
        }
...
export async function runLlmRewrite(text, effect, level, trueOriginal, respondingTo = '', recentMessages = [], ruleText = null, ruleAmount = null, primaryHitDirection = 'increase') {
    ...
    // ruleText !== null means this effect has rules configured (pipeline.js's applyEffects) — the
    // matched rule's own text IS the {{scale_instruction}} for this call, entirely replacing the
    // threshold-based scaleSteps lookup for it, same "rules take over once present" precedent the
    // activation gate already follows. No rules configured (ruleText === null) falls back to the
    // original scaleMode/scaleSteps behavior unchanged.
    const scaleInstruction = ruleText !== null
        ? ruleText
        : effect.llmRewrite.scaleMode === 'steps'
            ? resolveScaleStep(effect.llmRewrite.scaleSteps, level, primaryHitDirection)
            : '';
    // Separate axis from scaleInstruction above: a fixed preset ("how much creative freedom to
    // take"), laddered by level via its own amountSteps, independent of the style/prose
    // instruction. ruleAmount !== null means rules are configured — same "rules take over once
    // present" precedent scaleInstruction/ruleText already follow, including that a matched rule
    // whose ladder resolves to '' (no step reached, or reached step has no preset set) means no
    // instruction at all, not a fallback to the effect-level default.
    const amountInstruction = ruleAmount !== null ? ruleAmount : resolveAmountStep(effect.llmRewrite.amountSteps, level, primaryHitDirection);
    ...
    const prompt = effect.llmRewrite.promptTemplate
        ...
        .replaceAll('{{amount_instruction}}', amountInstruction)
