...
import { getEffectLevel, setEffectLevel, getEffectLocked, setEffectLocked, isPrerequisiteMet } from './chatState.js';
import {
...
// Applies one effect's raw 0-10 classification rating — the level/locked math itself lives in
...
// cumulative-lock effect ignores the rating entirely without writing anything.
export function applyLlmRating(effect, rating0to10, allEffects = []) {
    const prerequisiteMet = isPrerequisiteMet(effect, allEffects);
    debugLog(`applyLlmRating "${effect.label}": rating=${rating0to10}/10, mode=${effect.trigger.llmIntegrationMode}, levelBefore=${getEffectLevel(effect).toFixed(2)}${prerequisiteMet ? '' : ' (blocked — dependency not met)'}`);

    const wasLocked = getEffectLocked(effect);
    if (effect.trigger.llmIntegrationMode === 'cumulative-lock' && wasLocked) {
        debugLog(`applyLlmRating "${effect.label}": locked, ignoring rating`);
        return getEffectLevel(effect);
    }

    const { level: newLevel, locked: newLocked } = resolveLlmRatingUpdate(
        getEffectLevel(effect), wasLocked, rating0to10, effect.trigger, prerequisiteMet,
    );
    const level = setEffectLevel(effect, newLevel);
    debugLog(`applyLlmRating "${effect.label}": -> level=${level.toFixed(2)}`);
...
export async function runBatchedLlmDetectors(effects, allEffects = effects) {
    ...
    try {
        ...
        for (const effect of effects) {
            ...
            // that won't collide with another effect's answer.
            const rating = extractRating(cleaned, effect.id);
            if (rating !== null) {
                applyLlmRating(effect, rating, allEffects);
            } else {
...
export async function runDetectionTest(effect, sampleText) {
