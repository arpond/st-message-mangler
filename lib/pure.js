...
export function migrateEffectsToTrackers(settings, logFn = console.log) {
    ...
    for (const effect of settings.effects ?? []) {
        ...
        // `label` lived on the fused effect's top level, not under `.trigger` — carry it onto the
        // new Tracker too (real gap found post-migration: a split tracker with no label at all is
        // hard to identify in its own list). The Effect keeps its own copy of the same label;
        // either can be renamed independently afterward.
        const tracker = { id: effect.id, label: effect.label, chatActivationMode: effect.chatActivationMode ?? 'auto', ...trackerFields };
