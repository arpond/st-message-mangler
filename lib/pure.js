...
// Fills `count` steps (clamped to >= 1) with computed thresholds and blank text, for the
// generator button in the scale-step editor. 'linear' spaces them evenly across [0, 1];
// 'exponential' squares the linear position, clustering thresholds toward the low end (more
// bands at subtle levels, fewer/bigger jumps near the top).
export function generateScaleSteps(count, curve = 'linear') {
