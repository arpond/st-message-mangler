...
export function resolveLlmRatingUpdate(currentLevel, currentLocked, rating0to10, trigger) {
    if (trigger.llmIntegrationMode === 'absolute') {
        return { level: clamp01(rating0to10 / 10), locked: currentLocked };
    }
    const hit = rating0to10 >= trigger.llmHitThreshold;
...
export function resolveDetectionLevelUpdate(currentLevel, currentTurnsActive, detectionText, trigger) {
    ...
    if (trigger.detector === 'llm') {
        ...
        const hit = matchesKeywordList(detectionText, trigger.keywords);
