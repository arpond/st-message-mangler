...
export function defaultTrigger() {
    return {
        ...
        dependsOnEffectId: '', // '' = no dependency; otherwise this effect can't increase until the referenced effect's level >= dependsOnMinLevel
        dependsOnMinLevel: 0.5, // 0-1; only relevant when dependsOnEffectId is set
    };
}
...
export function defaultEffectShape(type = 'regex') {
    return {
        ...
        characterAvatar: '', // '' = unbound (matches any character, today's global behavior); otherwise this effect's detect/target relationship is always "that specific character <-> user" — group-chat-aware
...
// binding), independent of the existing detectSource/target axes. User messages have no
// character identity to bind against, so they always pass regardless of binding — this only ever
// restricts which AI character's messages count. `messageCharacterAvatar` is the caller's
// resolved identity for the message in question (see pipeline.js's resolveMessageCharacterAvatar,
// which handles the ST-specific fallback chain — this function only does the comparison).
export function matchesBoundCharacter(effect, source, messageCharacterAvatar) {
    if (source === 'user') return true;
    if (!effect.characterAvatar) return true;
    return effect.characterAvatar === messageCharacterAvatar;
}
...
export function resolveLlmRatingUpdate(currentLevel, currentLocked, rating0to10, trigger, prerequisiteMet = true) {
    ...
    const hit = prerequisiteMet && rating0to10 >= trigger.llmHitThreshold;
    const level = clamp01(currentLevel + (hit ? trigger.incrementPerHit : -trigger.decayPerTurn));
    const locked = currentLocked || (trigger.llmIntegrationMode === 'cumulative-lock' && level >= trigger.lockThreshold);
    return { level, locked };
}
...
export function resolveDetectionLevelUpdate(currentLevel, currentTurnsActive, detectionText, trigger, prerequisiteMet = true) {
    if (matchesKeywordList(detectionText, trigger.dispelKeywords)) {
        return { level: 0, turnsActive: 0, dispelled: true, autoDispelled: false };
    }
    ...
    if (trigger.detector === 'llm') {
        ...
        level = clamp01(currentLevel + (hit ? trigger.incrementPerHit : -trigger.decayPerTurn));
    }

    const active = level >= trigger.minLevelToApply;
...
// Walks candidateDependencyId's own dependency chain (following trigger.dependsOnEffectId) to
// check whether picking it as effectId's dependency would create a cycle — including depending
// directly on itself. A dangling reference mid-chain (an id that doesn't match any effect, e.g.
// after a deletion) just ends the walk without matching, not a false-positive cycle — that case
// is "broken reference", a separate concern handled elsewhere (lib/chatState.js).
export function wouldCreateCycle(effects, effectId, candidateDependencyId) {
    let currentId = candidateDependencyId;
    const visited = new Set();
    while (currentId) {
        if (currentId === effectId) return true;
        if (visited.has(currentId)) return false; // already-existing cycle elsewhere in the graph, not this one
        visited.add(currentId);
        const current = effects.find(e => e.id === currentId);
        currentId = current?.trigger?.dependsOnEffectId || '';
    }
    return false;
}
