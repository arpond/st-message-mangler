...
// Wall-clock HH:MM:SS, local time — event log timestamps are session-scoped (never persisted or
// compared across timezones), so this is just a readable label, not a stored/serialized format.
...
const CUE_LOG_TRUNCATE_LENGTH = 60;

// One human-readable line per event-log entry kind (lib/eventLog.js). Deliberately terse — this
// exists to fix the hidden debug flag's "hard to scan, no clear labeling of action/reason"
// feedback, so each line names the action and the reason in one glance rather than dumping the
// full pipeline trace. `full` is non-null only when the summary was actually truncated (cue text),
// so callers can skip a redundant tooltip otherwise.
export function describeLogEvent(event) {
    const { kind, detail } = event;
    switch (kind) {
        case 'level-change':
            return { summary: `Level ${detail.from.toFixed(2)} → ${detail.to.toFixed(2)} (${detail.reason})`, full: null };
        case 'dispel':
            return { summary: `Dispelled (${detail.reason})`, full: null };
        case 'auto-dispel':
            return { summary: `Auto-dispelled (${detail.reason})`, full: null };
        case 'lock':
            return { summary: `Locked at ${detail.level.toFixed(2)} (lock threshold ${detail.lockThreshold})`, full: null };
        case 'cue-injected': {
            const text = detail.text;
            const truncated = text.length > CUE_LOG_TRUNCATE_LENGTH ? `${text.slice(0, CUE_LOG_TRUNCATE_LENGTH)}…` : text;
            return { summary: `Cue: "${truncated}"`, full: truncated !== text ? text : null };
        }
        case 'manual-dispel':
            return { summary: 'Dispelled manually (status panel)', full: null };
        case 'manual-set-level':
            return { summary: `Level set manually → ${detail.to.toFixed(2)}`, full: null };
        case 'manual-active-toggle':
            return { summary: `Active toggled ${detail.active ? 'on' : 'off'} manually`, full: null };
        default:
            return { summary: kind, full: null };
    }
}
