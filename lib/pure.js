...
export function defaultRule() {
    return {
        ...
        conditions: [],
        text: '', // optional — becomes this llm-rewrite effect's {{scale_instruction}} when this
        // rule matches, entirely replacing the Structured-steps threshold lookup for it (same
        // placeholder, no separate one to reconcile in the template). Ignored by regex/drunk.
    };
}
...
export function resolveRuleOutput(rules, ruleMode, resolvedLevels, trackerById) {
    ...
    if (ruleMode === 'stack') {
        ...
        return { active: matched.length > 0, text: matched.map(r => r.text).filter(Boolean).join('\n\n') };
    }
    const first = rules.find(ruleMatches);
    return first ? { active: true, text: first.text } : { active: false, text: '' };
}
