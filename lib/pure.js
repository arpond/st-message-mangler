...
// extreme in hitDirection) regardless of currentLevel; 'increment' nudges by incrementPerHit in
// that direction. On no hit: always drifts back toward restingLevel by decayPerTurn — this is
// "decay", generalized to also work when resting is high (drifts upward instead of down).
export function resolveHitLevel(currentLevel, hit, trigger) {
    const sign = trigger.hitDirection === 'decrease' ? -1 : 1;
    if (hit) {
        return trigger.hitBehavior === 'jump' ? (sign > 0 ? 1 : 0) : clamp01(currentLevel + sign * trigger.incrementPerHit);
    }
    return restingLevelValue(trigger.restingLevel) === 1
        ? clamp01(currentLevel + trigger.decayPerTurn)
        : clamp01(currentLevel - trigger.decayPerTurn);
}
...
export function resolveLlmRatingUpdate(currentLevel, currentLocked, rating0to10, trigger, prerequisiteMet = true) {
    ...
    const level = resolveHitLevel(currentLevel, hit, trigger);
