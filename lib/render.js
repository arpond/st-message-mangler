import { escapeHtmlForDisplay, resolveAwarenessCue, resolveScaleStep, wouldCreateCycle, buildTrackerAutoCueTemplate, AMOUNT_PRESETS, describeLogEvent, formatEventTimestamp } from './pure.js';
...
// Session-only event log (lib/eventLog.js) rendered newest-first. `events` is this effect's
// tracker's own log (chronological, oldest first, as logEvent appends it) — reversed here so
// callers (both the per-effect Log tab and the status panel's trimmed recent-activity feed) never
// have to remember to do it themselves.
export function renderEventLogPanel(events) {
    if (events.length === 0) return '<small>No activity logged yet this session.</small>';
    ...
    return `<div class="st_mangler_log_list">${rows}</div>`;
}
