...
export function renderTriggerPanel(tracker, level, turnsActive, locked) {
    ...
    return `
        <div class="st_mangler_trigger" style="display: ${tracker.mode === 'progressive' ? 'block' : 'none'};">
            ...
            <label class="st_mangler_trigger_row">
                Min level to apply (below this, any Effect using this tracker stays dormant)${infoIcon('Also drives this tracker\'s own turns-active/auto-dispel bookkeeping below, independent of any specific Effect.')}
...
export function renderRulesPanel(effect, allTrackers = []) {
    ...
    const ruleBlocks = rules.map((rule, i) => {
        ...
        <div class="st_mangler_rule" data-rule-index="${i}">
            ...
            <label>
                Instruction text${infoIcon('Becomes this llm-rewrite effect\'s {{scale_instruction}} when this rule matches — the same placeholder Structured steps fills in, so no separate reconciliation is needed in the template. Ignored by regex/drunk effects (nothing to substitute it into).')}
    ...
    return `
        <div class="st_mangler_trigger_row">
            Rules${infoIcon('Optional. Each rule is an AND-gate over one or more tracker conditions. "First match wins" walks the list in order and uses the first fully-matching rule\'s instruction text; "Stack all matches" instead joins every matching rule\'s text together. While any rules exist, they entirely replace this effect\'s own tracker\'s Min level to apply gate, AND (for llm-rewrite) entirely replace Structured steps\' threshold-based {{scale_instruction}} lookup with whichever rule matched — the tracker picked on the Basics tab still supplies {{level}}/{{level_pct}}/{{trend}} and this effect\'s chat-activation/character-binding either way.')}
