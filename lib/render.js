...
// `allEffects` (every other configured effect) builds the dependency picker's option list,
// filtered via wouldCreateCycle so a cycle-forming choice can't be selected in the first place.
// `dependencyState` is lib/chatState.js's describeDependencyState(effect, allEffects) result —
// null (no dependency, or satisfied), or { broken, reason } for the inline status line. Only
// meaningful for progressive effects — 'always' mode has no escalation to gate.
export function renderDependencyPanel(effect, allEffects = [], dependencyState = null) {
    ...
    return `
        <label>
            Depends on effect${infoIcon("Blocks this effect's level from increasing until another effect reaches a minimum level — decay/dispel still apply normally while blocked. Optional; leave as \"None\" for no dependency.")}
            <select class="st_mangler_field" data-field="trigger.dependsOnEffectId">
                <option value="" ${!effect.trigger.dependsOnEffectId ? 'selected' : ''}>None</option>
                ${allEffects.filter(e => e.id !== effect.id && !wouldCreateCycle(allEffects, effect.id, e.id))
                    .map(e => `<option value="${e.id}" ${effect.trigger.dependsOnEffectId === e.id ? 'selected' : ''}>${escapeHtmlForDisplay(e.label || e.id)}</option>`)
                    .join('')}
            </select>
        </label>
        <label style="display: ${effect.trigger.dependsOnEffectId ? 'block' : 'none'};">
            Min level required:
            ${field('number', 'trigger.dependsOnMinLevel', effect.trigger.dependsOnMinLevel, 'step="0.01" min="0" max="1" style="max-width: 6em;"')}
        </label>
        ${dependencyState ? `
        <small class="${dependencyState.broken ? 'st_mangler_warning' : ''}">
