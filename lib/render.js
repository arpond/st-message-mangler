import { escapeHtmlForDisplay, resolveAwarenessCue, resolveScaleStep, wouldCreateCycle, buildTrackerAutoCueTemplate } from './pure.js';
...
export function renderTriggerPanel(tracker, level, turnsActive, locked) {
    ...
    // Same mirrored-threshold bug class as minLevelLabel above — "once level reaches this" reads
    // literally correct only for Increase; under Decrease the tracker actually locks once level
    // has fallen to 1 - lockThreshold, not once it "reaches" the entered number.
    const lockThresholdLabel = decreasing
        ? 'Lock threshold (0-1) — once the level has fallen this far toward 0, it stops drifting back permanently until dispelled:'
    ...
    const ruleBlocks = rules.map((rule, i) => {
        ...
        <div class="st_mangler_rule" data-rule-index="${i}">
            ...
            ${effect.type !== 'llm-rewrite'
                ? `<small>This effect has no transform (${EFFECT_TYPE_LABELS[effect.type] ?? effect.type}) — this rule's conditions still gate its activation/awareness cue above, but there's no {{scale_instruction}} to fill, so Instruction text is hidden here.</small>`
